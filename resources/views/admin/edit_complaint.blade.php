                ...
                <form action="{{ route('complaint.update', $complaint->id) }}" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <select id="inputJob" class="form-control input-style" name="job_id">
                            ...
                            @foreach ($jobs as $job)
                                <option value="{{ $job->id }}" @if($job->id == $complaint->job_id) selected @endif>{{ $job->name }}</option>
                            @endforeach
                    ...
                    <div class="form-group">
                        ...
                        <select id="utility_id" class="form-control input-style" name="utility_id">
                            ...
                            @foreach ($utilities as $utility)
                                <option value="{{ $utility->id }}" @if($utility->id == $complaint->utility_id) selected @endif>{{ $utility->name }}</option>
                            @endforeach
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control input-style" name="description" id="description" rows="2" 
                        placeholder="Description" required >{{ $complaint->description }}</textarea>
                      </div>
                   
                    <button type="submit" class="btn btn-primary btn-style mt-4">Update complaint</button>
                </form>
            </div>
        </div>
        <!-- //forms 1 -->
